...
def data_loader(batch_size=128, data_path='content'):
    # load data from csv file
    fields = ['news_article', 'news_category']

    train_data = pd.read_csv(os.path.join(data_path, 'inshort_news_data-train.csv'), header=0, encoding='ISO-8859-1',
                             usecols=fields, skip_blank_lines=True)
    val_data = pd.read_csv(os.path.join(data_path, 'inshort_news_data-val.csv'), header=0, encoding='ISO-8859-1',
                           usecols=fields, skip_blank_lines=True)
    test_data = pd.read_csv(os.path.join(data_path, 'inshort_news_data-test.csv'), header=0, encoding='ISO-8859-1',
                            usecols=fields, skip_blank_lines=True)
    ...
    stopwords = []
    stop = open(os.path.join(data_path, 'stopwords.txt'), encoding="utf-8")
    for line in stop:

...
def data_loader_new(train_dataloader, val_dataloader, test_dataloader, batch_size=128):
    ...
    for i in range(X_train.shape[0]):
      ...
      for j in range(len(X_train[i])):
        if not (X_train[i][j].lower() in stopwords):
          embedding_matrix_train[i][index] = glove[X_train[i][j].lower()]
          index += 1
    ...
    for i in range(X_val.shape[0]):
      ...
      for j in range(len(X_val[i])):
        if not (X_val[i][j].lower() in stopwords):
          embedding_matrix_val[i][index] = glove[X_val[i][j].lower()]
          index += 1
    ...
    for i in range(X_test.shape[0]):
      ...
      for j in range(len(X_test[i])):
        if not (X_test[i][j].lower() in stopwords):
          embedding_matrix_test[i][index] = glove[X_test[i][j].lower()]
          index += 1
